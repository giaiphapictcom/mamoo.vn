...
function Admin_ImagesGroup_Xoa(pId, pPage) {
    ...
    $.ajax({
        ...
        data: { 'pId': pId },
        dataType: 'json',
        url: "/Admin/ThucHienXoaTheLoaiAnh",
        timeout: 60000,
        success: function (data) {
            if (data.code == 1) {
                window.location = "/Admin/TheLoaiAnh?pPage=" + pPage + "";
            }
            else {
...
function Admin_ImagesGroup_ThucHienLuuMoi() {
    ...
    $.ajax({
        ...
        dataType: 'json',
        url: "/Admin/TheLoaiAnhThucHienThemMoi/",
        timeout: 60000,
        success: function (data) {
            $("#wait").css("display", "none");
            if (data.code == 1) {
...
function Admin_ImagesGroup_ThucHienLuuSua() {
    ...
    $.ajax({
        ...
        dataType: 'json',
        url: "/Admin/TheLoaiAnhThucHienSuaAnh/",
        timeout: 60000,
        success: function (data) {
            $("#wait").css("display", "none");
            if (data.code == 1) {
...
function Admin_ImagesGroup_ComeHome() {
    ...
    window.location = "/Admin/TheLoaiAnh";
}

function Admin_ImagesGroup_ComeSua(pId) {

    window.location = "/Admin/TheLoaiAnhSua?pId=" + pId + "";
}
function ThayDoiKieuNhomAnh2(pPage) {
    var pType = $("#pGroupIdHome").val();
    window.location = "/Admin/TheLoaiAnh?pPage=1&pType=" + pType + "";
}