...
function Admin_FileGroup_Xoa(pId, pPage) {
    ...
    $.ajax({
        ...
        data: { 'pId': pId },
        dataType: 'json',
        url: "/Admin/ThucHienXoaTheLoaiFile",
        timeout: 60000,
        success: function (data) {
            if (data.code == 1) {
                window.location = "/Admin/TheLoaiFile?pPage=" + pPage + "";
            }
            else {
...
function Admin_FileGroup_ThucHienLuuMoi() {
    ...
    $.ajax({
        ...
        dataType: 'json',
        url: "/Admin/TheLoaiFileThucHienThemMoi/",
        timeout: 60000,
        success: function (data) {
            $("#wait").css("display", "none");
            if (data.code == 1) {
...
function Admin_FileGroup_ThucHienLuuSua() {
    ...
    $.ajax({
        ...
        dataType: 'json',
        url: "/Admin/TheLoaiFileThucHienSua/",
        timeout: 60000,
        success: function (data) {
            $("#wait").css("display", "none");
            if (data.code == 1) {
...
function Admin_FileGroup_ComeHome() {
    ...
    window.location = "/Admin/TheLoaiFile";
}

function Admin_FileGroup_ComeSua(pId) {

    window.location = "/Admin/TheLoaiFileSua?pId=" + pId + "";
}
function ThayDoiKieuNhomFile2(pPage) {
    var pType = $("#pGroupIdHome").val();
    window.location = "/Admin/TheLoaiFile?pPage=1&pType=" + pType + "";
}